...
- [karpathy/autoresearch](https://github.com/karpathy/autoresearch)：「评测不可被优化对象篡改」原则（`prepare.py` 只读）与量化实验台账（`results.tsv`，对应本框架的事件流与循环度量）。
...
| Level 1.5（配置层） | Meta-Analyst | 运行配置（白名单内） | 主控追加 `config` 事件应用 |
...
1. 自上次元分析起已过 **5 轮**；
2. 本轮有任务进入 `blocked`；
3. 循环度量中同一失败类别累计出现 **≥3 次**。

派发指令："读 `prompts/meta-analyst.md` 并遵守其中规则，然后基于 `loop/state.md`（投影）、`loop/events.jsonl` 与 `loop/` 下的归档回执/裁决完成元分析，写出 `loop/meta/round<N>.md`"（N 为当前轮次）。

主控读回报告后：白名单内的「配置调整」逐项追加 `config` 事件（`source` 填该报告路径），越出白名单的调整拒绝并记备忘；投影重算时配置节随之更新。
...
| 每批并行上限 | 3 | 1–3 |
| 每任务重试上限 | 2 | 0–3 |
| 每会话 tick 上限 | 3 | 1–5（调小 = 主控更早交接更保守; 降级串行模式下主控自行固定为 1, 不受此项调整） |
| lessons 注入 Worker 简报 | 开 | 开 / 关 |
| 验证强度 | 标准 | 标准 / 严格（严格 = 验证简报中要求 Verifier 通读全部产出物而非抽查；`risk: high` 任务强制严格，不受此项调低影响） |

白名单之外一切不可调：max_ticks 保险丝（只有人类可改，经 inbox 指令）、验收标准、协议与提示词文件、TASKS.md。当前值由 `config` 事件重放得出，并展示在 state.md 投影的「运行配置」节。
...
## 轨迹诊断
<2–5 行: 从度量表与事件流中观察到的模式。例:
 "3 个任务共 5 次失败, 4 次属 acceptance 且都栽在同一条模糊标准上">

## 配置调整  <!-- 仅白名单项; 无调整写"无" -->
- 每批并行上限: 3 → 1（原因: 两次 constraint 失败均发生在并行批中）
...
2. Meta-Analyst **不读产出物、不跑命令**，只依据 state.md 投影（度量 + 配置）、events.jsonl（轨迹）与归档的回执/裁决。它分析的是循环的轨迹，不是任务的内容。
3. Meta-Analyst 只写 `loop/meta/` 与 `loop/lessons.md`，其余一切只读（配置变更由主控以 `config` 事件执行）。
