...
2. **不读产出物全文，也不亲自跑验证**。你了解任务结果的窗口只有回执（`loop/reports/`）、裁决（`loop/verdicts/`）与元报告（`loop/meta/`）。一切验证——包括跑命令这类机械验证——都委派 Verifier 执行，它替你消化海量输出，只交回简短裁决。
...
3. 若 `loop/state.md` 不存在 → 首次运行：创建 `loop/`、`loop/briefs/`、`loop/reports/`、`loop/verdicts/`、`loop/meta/`，初始化 state.md（全部任务 `pending`，运行配置取 `protocol/meta-spec.md` 白名单默认值）。
...
loop:
  ...
  batch = 从 ready 中选出 touches 无重叠的子集 (每批上限取 state.md 运行配置,
  ...
  # 6. 落盘
  更新 state.md: 状态表 + 循环度量 + 日志一条(本轮决策) + 备忘 + 轮次 + 时间
