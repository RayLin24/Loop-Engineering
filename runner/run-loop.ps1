...
function Get-LoopStatus {
    ...
    $line = Select-String -Path $stateFile -Pattern '^\s*-\s*状态:\s*(\S+)' | Select-Object -First 1
...
function Get-TickNumber {
    ...
    $line = Select-String -Path $stateFile -Pattern 'tick:\s*(\d+)' | Select-Object -First 1
...
for ($gen = 1; $gen -le $MaxGenerations; $gen++) {
    ...
    switch ($status) {
        ...
        "waiting-human" {
            Write-Host "[runner] 循环等待人工介入: 看 loop/state.md 的备忘与 loop/questions/ 待答问题。处理后重新运行本脚本。" -ForegroundColor Yellow
            exit 2
        }
    }

    $tickBefore = Get-TickNumber
    $prompt = if ($status -eq "fresh") { "按 LOOP.md 接管任务清单" } else { "按 LOOP.md 继续" }
    Write-Host "[runner] 第 $gen 代: claude -p `"$prompt`"  (状态: $status, tick: $tickBefore)" -ForegroundColor Cyan

    & claude -p $prompt @permArgs
    # 会话本身失败(非零退出)不立即终止——按停滞检测处理, 给瞬时故障一次机会

    $tickAfter = Get-TickNumber
    if ($tickAfter -le $tickBefore) {
        $stallCount++
        Write-Host "[runner] 警告: 本代 tick 未前进 ($tickBefore -> $tickAfter), 停滞计数 $stallCount/2。" -ForegroundColor Yellow
        if ($stallCount -ge 2) {
